...
Você deverá criar um repositório como o repo principal do projeto, i.e., todos os seus commits devem estar registrados, pois queremos ver como você trabalha.
...
Você precisa criar o ambiente de execução desta API com o maior número de passos automatizados possível, inclusive a esteira de deploy. A API NÃO deve ficar indisponível durante a atualização via esteira de deploy.

A aplicação será uma API REST que está disponível nesta pasta. Através dela os internautas enviam comentários em texto de uma máteria e acompanham o que outras pessoas estão falando sobre o assunto em destaque. O funcionamento básico da API consiste em uma rota para inserção dos comentários e uma rota para listagem.
